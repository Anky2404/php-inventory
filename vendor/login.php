...
if(count($errors) == 0){
    ...
    $query = "SELECT * FROM `users` WHERE `name` = '$name' AND `password` = '$pass' AND `role` = 4";
    $result = mysqli_query($connect,$query);

    if($rows = mysqli_fetch_assoc($result)) {
       
        $_SESSION['vendor'] = $rows;

        if(isset($_SESSION['vendor'])){

            $_SESSION['success_message'] = 'Welcome ! Back To Vendor-Panel';
